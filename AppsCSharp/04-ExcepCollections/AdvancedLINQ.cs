    ...
    public static void AdvancedLINQ()
    {
      ...
      [
        ...
        foreach(var team in teamsMethod)
        {
          ...
          {
            WriteLine($" - {member.Alias} ({member.Name})");
          }
        }
        // Unir colecciones
        var charactersWithAbilities = from c in characters
                                      ...
                                      select new
                                      {
                                        c.Alias, c.Name, a.Description
                                      };
                                      WriteLine("\n🦸‍♂️ Personajes y sus habilidades:");
        foreach(var item in charactersWithAbilities)
        {
          WriteLine($" - {item.Alias} ({item.Name}): {item.Description}");
        }
        int totalPower = statistics.Sum(s => s.Power);
        WriteLine($"\n⚡ Poder total de todos los personajes: {totalPower}");
        ...
        WriteLine("\n📝 Cantidad de habilidades por personaje:");
        foreach(var item in abilitiesByCharacter)
        {
          WriteLine($" - {item.Character}: {item.Count} habilidades");
        }
